...
VERIFICATION_PROMPT = """你是一个专业的商标侵权检测专家。请分析以下淘宝商品是否侵犯"中国传媒大学"的商标权。
...
### 构成侵权的典型特征（满足任意多项）：
1. **商标使用**：标题或描述中使用了"中国传媒大学"、"中传"、"CUC"、"广院"等校名/缩写
2. **商品类别**：属于常见的侵权周边类别（服装、文具、饰品、纪念品、手机壳等）
3. **暗示官方关联**：使用"官方"、"正版"、"授权"、"纪念"、"周边"、"文创"等词暗示与学校有关联
4. **价格异常**：价格明显低于正常授权商品的市场价
5. **卖家非官方**：卖家不是学校官方店铺或授权经销商

### 不构成侵权的典型特征：
1. **合理使用**：仅提及学校名称但商品本身与学校无关（如"中国传媒大学考研资料"）
2. **二手/闲置**：明确标注为二手、闲置的个人物品转让
3. **官方店铺**：卖家为中国传媒大学官方或明确授权的店铺
4. **书籍/教材**：正规出版的教材、书籍（属于合理使用范畴）
...
def _parse_ai_response(content):
    ...
    return {
        ...
        "suggestion": result.get("suggestion", "人工复核"),
    }
...
def verify_single_product(title, price="", seller="", location="", progress_cb=None):
    ...
    if progress_cb:
        ...
        progress_cb(f"   AI结果: {status} (置信度:{result['confidence']}) - {result['reason']}")
...
def ai_enhanced_judgment(title, price, location, rule_result, rule_reason, progress_cb=None):
    ...
    if ai_result["is_infringement"] is True:
        # AI判定侵权 → 采纳
        return True, f"AI判定侵权(置信度:{ai_result['confidence']}): {ai_result['reason']}", True
